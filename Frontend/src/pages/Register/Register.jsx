...
export default function Register () {
  ...
  const validate = () => {
    const cur = { name: '', email: '', password: '', global: '' }
    if (!form.name.trim()) cur.name = 'Name is required.'
    if (!form.email.trim()) cur.email = 'Email is required.'
    else if (!emailRegex.test(form.email)) cur.email = 'Invalid email format.'
    if (!form.password.trim()) cur.password = 'Password is required.'
    else if (form.password.length < 6)
      cur.password = 'Password must be at least 6 characters long.'

    setErrors(cur)
    return !cur.name && !cur.email && !cur.password
  }
  ...
  return (
    <div>
      <div className='container w-50 flex flex-col justify-center '>
        <div className='p-5 rounded-lg mt-3'>
          <div className='flex flex-col items-center gap-2 mb-5'>
            <h2 className='text-[42px] font-bold'>Register</h2>
            <h4 className='text-[22px] font-light'>
              If you don&apos;t have an account, Go on and sign up
            </h4>
          </div>
